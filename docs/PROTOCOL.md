...
| `macros` | `items[]` = `{id,label,prompt}` | The macro deck contents (global) |
| `cfgopts` | `models[]` = `{id,label}`, `efforts[]` = `{id,label}` | Preset choices for the per-session config sheet (gear on a session-picker row). Sent in the post-auth snapshot; server-driven so new models never need a firmware update. |
| `alert` | `sid`, `level` (`info`/`attn`/`error`), `text`, `sound` (`chirp`\|`soft`\|`error`) | Pull-attention: flash + play the named built-in tone. The `sound` field is **omitted** when a sound clip is available for `alert.*` (for `attn`) or `done.*` — the bridge streams that PCM clip instead, and dropping the tone field keeps the tone and the clip from double-playing. The bridge ships with default clips (`bridge/sounds/`), so clips are the normal case; `~/.claudeq/sounds/` overrides them per user, and the synth tones remain the fallback if the clips are missing or ffmpeg is unavailable. |
...
| `setcfg` | `sid`, `model` **or** `effort` (an id from `cfgopts`) | Reconfigure a specific session (not necessarily the focused one): the bridge types `/model X` or `/effort Y` into that session's tmux pane. Values outside the `cfgopts` presets are ignored. One setting per message. |
